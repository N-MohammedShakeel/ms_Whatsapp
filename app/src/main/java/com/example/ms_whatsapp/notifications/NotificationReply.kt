...
class NotificationReply  : BroadcastReceiver(){
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (remoteInput!=null){
            ...
            // this can be further improved if user in other chatroom and message comes, he can use notification reply to send that exact room

            //this is for recent chats
            ...
            firestore.collection("Conversation${Utils.getUidLoggedIn()}").document(friendid).set(setHashap)

            val updateHashMap = hashMapOf<String, Any>("message" to repliedText, "time" to Utils.getTime(), "person" to friendname!!)

            firestore.collection("Conversation${friendid}").document(Utils.getUidLoggedIn()).update(updateHashMap)
